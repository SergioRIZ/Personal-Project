...
const Amigoball = ({ size = "w-24 h-24" }) => {
  ...
  return (
    ...
    >
      {/* Green top half */}
      <div className={`absolute top-0 left-0 w-full h-1/2 bg-green-600 rounded-t-full overflow-hidden transition-all duration-300 ${isHovered ? 'bg-green-700' : ''}`}>
        {/* Shine effect */}
        ...
        {/* Light green circles on top */}
        <div className="absolute top-4 left-1/5 w-4 h-4 bg-green-300 rounded-full"></div>
        <div className="absolute top-4 right-1/5 w-4 h-4 bg-green-300 rounded-full"></div>
        
        {/* A letter (simplified) */}
        <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 text-xl font-bold text-white">A</div>
      </div>
      ...
      {/* Center button */}
      <div className={`absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-7 h-7 bg-white rounded-full border-4 border-green-700 z-10 transition-all duration-300 ${isHovered ? 'scale-110 bg-gray-100' : ''}`}>
        {/* Button press animation */}
        <div className={`absolute inset-0 bg-green-200 rounded-full scale-0 transition-transform duration-300 ${isHovered ? 'scale-50 animate-ping' : ''}`}></div>
      </div>
      ...
      {/* Special Amigoball effect: sparkles */}
      {isHovered && (
        <>
